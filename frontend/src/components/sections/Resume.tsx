...
const DEFAULT_HIGHLIGHTS = [
  { category: 'experience', icon: Briefcase,     title: 'Experience',  color: '#6c63ff' },
...
function ResumeMockup() {
  return (
    ...
    >
      {/* Shadow pages */}
      <div style={{ position: 'absolute', top: '12px', left: '12px', right: '-12px', bottom: '-12px', borderRadius: '16px', background: 'rgba(108,99,255,0.08)', border: '1px solid rgba(108,99,255,0.12)' }} />
      <div style={{ position: 'absolute', top: '6px', left: '6px', right: '-6px', bottom: '-6px', borderRadius: '16px', background: 'rgba(108,99,255,0.12)', border: '1px solid rgba(108,99,255,0.18)' }} />

      {/* Main page */}
      <div style={{ position: 'relative', borderRadius: '16px', background: 'var(--bg-surface)', border: '1px solid rgba(108,99,255,0.3)', padding: '1.75rem', boxShadow: '0 20px 60px rgba(108,99,255,0.18)' }}>
        <div style={{ height: '3px', background: 'linear-gradient(90deg, #6c63ff, #00d4ff)', borderRadius: '2px', marginBottom: '1.25rem' }} />

        <div style={{ marginBottom: '1rem' }}>
          <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '16px', color: 'var(--text-1)' }}>Sandip Gupta</div>
          <div style={{ fontSize: '11px', fontFamily: 'monospace', color: '#6c63ff', marginTop: '2px' }}>AI Engineer & Master Trainer</div>
        </div>
        ...
        {['Experience', 'Skills', 'Projects', 'Education'].map((s, i) => (
          <div key={s} style={{ marginBottom: '12px' }}>
            <div style={{ fontSize: '8px', fontFamily: 'monospace', color: '#6c63ff', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '5px' }}>
              <div style={{ height: '1px', flex: 1, background: 'rgba(108,99,255,0.25)' }} />
              {s}
              <div style={{ height: '1px', flex: 1, background: 'rgba(108,99,255,0.25)' }} />
            </div>
...
export default function Resume() {
  ...
  return (
    <section id="resume" style={{ padding: '6rem 1.5rem', position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '30%', right: '5%', width: '300px', height: '300px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(108,99,255,0.06), transparent)', filter: 'blur(60px)' }} />
      </div>
      ...
      <div style={{ maxWidth: '72rem', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
          ...
          <FadeUp delay={0.1}>
            <p style={{ color: 'var(--text-2)', fontSize: '15px', maxWidth: '36rem', margin: '0 auto', lineHeight: 1.7 }}>
              2+ years building and teaching AI. Here's the condensed version.
            </p>
          </FadeUp>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {highlights.map((section, si) => (
                <motion.div key={section.title}
                  ...
                  whileHover={{ borderColor: section.color + '40', x: 4 }}
                  style={{ padding: '1.25rem', borderRadius: '14px', background: 'var(--bg-surface)', border: '1px solid var(--bd)', transition: 'border-color 0.2s' }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                    ...
                    <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '14px', color: 'var(--text-1)' }}>{section.title}</span>
                  </div>
        ...
        {/* Contact nudge */}
        <FadeUp delay={0.2}>
          <div style={{ marginTop: '4rem', padding: '2rem', borderRadius: '20px', background: 'linear-gradient(135deg, rgba(108,99,255,0.07), rgba(0,212,255,0.04))', border: '1px solid rgba(108,99,255,0.18)', textAlign: 'center' }}>
